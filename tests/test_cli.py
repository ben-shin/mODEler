...
def test_compare_global_observables_command_with_config_writes_outputs(tmp_path):
    data_path = tmp_path / "hsqc.csv"
    output_dir = tmp_path / "global_observable_model_comparison"
    config_path = tmp_path / "global_observable_model_comparison_config.json"
    ...
    assert (output_dir / "global_observable_model_comparison.csv").exists()
    assert (output_dir / "global_observable_model_comparison_failures.csv").exists()
    assert (output_dir / "peak_filtering.csv").exists()
    ...
    assert (best_fit_dir / "fitted_observables.csv").exists()
    assert (best_fit_dir / "residuals.csv").exists()
    assert (best_fit_dir / "model_metadata.json").exists()
    assert (best_fit_dir / "fit_result_summary.json").exists()
    assert (best_fit_dir / "run_metadata.json").exists()

    comparison = pd.read_csv(output_dir / "global_observable_model_comparison.csv")
...
def test_multistart_compare_global_observables_command_with_config_writes_outputs(
    tmp_path,
):
    data_path = tmp_path / "hsqc.csv"
    output_dir = tmp_path / "global_observable_multistart_model_comparison"
    config_path = tmp_path / "global_observable_multistart_model_comparison_config.json"
    ...
    assert (output_dir / "global_observable_multistart_model_comparison.csv").exists()
    assert (
        output_dir / "global_observable_multistart_model_comparison_failures.csv"
    ).exists()
    assert (output_dir / "global_observable_multistart_model_summary.csv").exists()
    assert (output_dir / "peak_filtering.csv").exists()
    ...
    comparison = pd.read_csv(
        output_dir / "global_observable_multistart_model_comparison.csv"
    )
...
def test_multistart_global_observables_variable_projection_command_writes_outputs(
    tmp_path,
):
    model_path = tmp_path / "model.txt"
    data_path = tmp_path / "hsqc.csv"
    output_dir = tmp_path / "global_hsqc_variable_projection_multistart"
    config_path = tmp_path / "global_hsqc_variable_projection_multistart_config.json"
    ...
    assert (output_dir / "variable_projection_multistart_comparison.csv").exists()
    assert (
        output_dir / "variable_projection_multistart_starting_parameters.csv"
    ).exists()
    assert (output_dir / "variable_projection_multistart_failures.csv").exists()
    assert (output_dir / "peak_filtering.csv").exists()
    ...
    comparison = pd.read_csv(
        output_dir / "variable_projection_multistart_comparison.csv"
    )
    ...
    parameters = pd.read_csv(best_fit_dir / "projected_fitted_parameters.csv")
