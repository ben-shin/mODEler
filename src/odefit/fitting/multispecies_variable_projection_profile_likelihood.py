...
@dataclass
class MultispeciesProfileLikelihoodFailure:
    parameter: str
    fixed_value: float
    error_type: str
    error_message: str
...
@dataclass
class MultispeciesProfileLikelihoodResult:
    original_result: MultispeciesVariableProjectionResult
    profile_table: pd.DataFrame
    failures: list[MultispeciesProfileLikelihoodFailure]
...
def fit_multispecies_variable_projection_profile_likelihood(
    *,
    model: ModelSpec,
    dataset: Dataset,
    parameter_specs: list[ParameterSpec],
    ...
    for parameter in profile_parameters:
        ...
        for index, fixed_value in enumerate(grid):
            if show_progress:
                print(
                    f"Multispecies profile likelihood {parameter}: "
            ...
            try:
                ...
                failures.append(
                    MultispeciesProfileLikelihoodFailure(
                        parameter=parameter,
                        fixed_value=float(fixed_value),
    ...
    return MultispeciesProfileLikelihoodResult(
...
def export_multispecies_profile_likelihood_result(
    *,
    result: MultispeciesProfileLikelihoodResult,
    output_dir: str | Path,
    ...
    table_path = output_path / "multispecies_profile_likelihood.csv"
    result.profile_table.to_csv(table_path, index=False)
    written_files["profile_likelihood"] = table_path

    metadata_path = output_path / "multispecies_profile_likelihood_metadata.json"
    ...
    if result.failures:
        failures_path = output_path / "multispecies_profile_likelihood_failures.csv"
