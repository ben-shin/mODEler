...
def _make_data_file(tmp_path):
    ...
    path = tmp_path / "numba_projection_workflow_data.csv"
...
def test_numba_projection_engine_runs_through_api_fit(tmp_path):
...
def test_numba_projection_engine_matches_reference_workflow_reasonably(tmp_path):
    data_path = _make_data_file(tmp_path)
    ...
    numba_output = fit_global_observables_from_config(
        _fit_config(
            data_path=data_path,
            engine_name="numba_projection",
        )
    ...
    numba_result = numba_output["result"]

    assert reference_result.success
    assert numba_result.success

    assert np.isclose(
        numba_result.fitted_parameters["k1f"],
